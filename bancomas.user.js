...
// 
// ,a.addEventListener("load",K,!1) удалено, т.к. в Chrome+TamperMonkey через раз вызывало Illegal invocation
// оно же // A fallback to window.onload, that will always work  Illegal invocation 
//        window.addEventListener( "load", completed, false );
// 
// 
// ,"function"==typeof define&&define.amd&&define("jquery",[],function(){return n}) удалено, т.к. возникали проблемы с require.js
// оно же if ( typeof define === "function" && define.amd ) {
...
function insertIntoTextarea (type, string_extra, area_filter, url)
{ 
    var area = $(area_filter)[0];
...
function addAditionalSearch (type)
{
    ...
    var bankiSearchQuery = 'http://www.banki.ru/forum/?PAGE_NAME=search&topic_id=&how=d&q=';
    ...
    switch (type) { 
        case 'forum_search': 
            var searchElemsHTML = "<a href='javascript:'><img src='"+prefix+"src' id="+prefix+"inThemeSearchType style='position:relative; top:2px; right:5px'></a><input placeholder='Поиск по теме' class='input--search' name='"+prefix+"input_search' type='text' style='height: 24px;' size=30>"; 
            searchElemsHTML = searchElemsHTML.replace(prefix+'src',getSearchIcon(searchType));
            if (!page.oldDesign) $("div.forum-header-search+span").replaceWith(searchElemsHTML)
            else $(".forum-header-options").first().prepend(searchElemsHTML);
            if (searchType === 'banki') { 
                searchQuery = bankiSearchQuery.replace('topic_id=&','topic_id='+page.params['TID']+'&');
            }
            else { searchQuery = googleSearchQuery + 'inurl:TID=' + page.params['TID']+' '; }    
            break;
        case 'responces' : 
            var searchElemsHTML = "<span class='inline-element-item'><input type='text' name='"+prefix+"input_search' placeholder='Поиск по отзывам' size='30'><input type='button' style='position:relative; left:-38px' class='input-search__button input-search__button--enabled' name='"+prefix+"button_search'></span>"; 
            $("DIV.inline-elements").append(searchElemsHTML);
    ...
    $("[name='"+prefix+"input_search']").attr('data-type','banki');
    ...
    function additionalSearchSubmit(e) {
        ...
        var ancor = '';
        if ( $("[name='"+prefix+"input_search']").attr('data-type') === 'banki') { ancor ='#search-result'; }
        
        var searchWindow = window.open($("[name='"+prefix+"input_search']").attr('data-src')+input+ancor);
        searchWindow.focus();
    }
    ...
    $('#'+prefix+'inThemeSearchType').on('click', function() {
        ...
        $("[name='"+prefix+"input_search']").attr('data-src',searchQuery);
        $("[name='"+prefix+"input_search']").attr('data-type',searchType);
    });
...
// выбираем весь текст в элементе, если ничего не выбрано
function selectAllIfNoSelection(elem) {
...
page.showErrors = function(err, global) {
    if ((/banki\.ru/.test(err.stack))||(/yandex\.st/.test(err.stack))) { return;  }  // ошибка сайта? плевать на нее!
    var showErrors = getParam('showErrors');
    err.stack = err.stack.replace(/file([\s\S]*?)bancomas\.user\.js/gi, 'bancomas.user.js');
...
page.addHrefsToHP = function() {
    ...
    var DIV_COMMENT_TEXT_FILTER = "div.comment__text";
    var A_NICK_FILTER = "a.comment__author";
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        $(ARTICLE_COMMENT_FILTER).each(function() {
            var article = this;
            if ($(article).find(SPAN_QUOTE_FILTER).length>0) return;
            var nick = $(article).find(A_NICK_FILTER).text();
...
/* Элемент, содержащий отзыв */
var FILTER_ARTICLE_EXPAND = 'article.responses__item';
/* Элемент, содержащий все отзывы */
var FILTER_DIV_RESPONCES_LIST = 'div[data-responses-list]';
...
page.removeFeedbackButton = function() {
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        $(FILTER_DIV_FEEDBACK_PANEL).remove();
        this.disconnect();
    });
...
page.addLastVisit = function() {
    $.ajax({url: '/forum/', dataType:'text', async:true, crossDomain: true}).done(function(responce) {
        $(FILTER_DIV_FORUM_USER_ADDINIONAL).append(function(){
            ...
            var regForumStat = new RegExp('<div class="forum-info-box forum-users-online">[\\s\\S]*?UID='+userId+'"[\\s\\S]*?<div class="forum-info-box forum-users-birthday">');
            var ret;
...
/** Добавляет в закрытые темы ссылку на цитирование  сообщения */
page.addQuotesToClosedThemes = function() {
    
    var A_TEXT = 'Цитировать';
...
var A_REPLY_FILTER = ".forum-quote a[href*='#message']";
var A_ANCHOR_FILTER = "div>table:has(a[name*='%s'])";
/** Делает ссылки 'пишет' относительными, если есть якорь на странице */
page.changeReplyHrefs = function() {
    $(A_REPLY_FILTER).each(function() {
      if (!/^.*?#(message\d+)$/.test($(this).attr('href'))) return;
      var afterHash = /^.*?#(message\d+)$/.exec($(this).attr('href'))[1];    
      if ($(A_ANCHOR_FILTER.replace('%s', afterHash)).length===0) return;
      $(this).attr('href', '#'+afterHash); $(this).attr('target', '');
      $(this).on('mousedown', function() {
          window.location.href='#'+afterHash;
          $.scrollTo(A_ANCHOR_FILTER.replace('%s', afterHash));
      });
    });
}
...
page.addHotKeysToForum.nameForUser = 'Заменять в форуме переход по страницам по Ctrl <- ->';

var FORUM_PAGINATION_CHANGE_ELEM_FILTER = '.forum-pagination-top, .forum-pagination-bottom';
var SCRIPT_PAGINATION_CHANGE_ELEM_CLASS = prefix+'forum-page-next';
page.addGotoPage = function()
{
    if ($(FORUM_PAGINATION_CHANGE_ELEM_FILTER).length === 0) return false;

    $(FORUM_PAGINATION_CHANGE_ELEM_FILTER).replaceWith("<span><a class='va-baseline forum-page-next "+SCRIPT_PAGINATION_CHANGE_ELEM_CLASS+"'>Перейти к:</a> <input class='"+SCRIPT_PAGINATION_CHANGE_ELEM_CLASS+"' size='4' style:'heigth=24px;'></span>");
    
    function prepareToChange(newPage)
    {
        if (isNaN(newPage)) { return false; }
        oldpageRegexp = new RegExp("&PAGEN_1=\\d+");
        messageRegexp = new RegExp("&MID=\\d+");
        if (newPage>0) { changePage(window.location.href, oldpageRegexp, messageRegexp, '&PAGEN_1=', newPage); }
    
    }
    
    $('.'+SCRIPT_PAGINATION_CHANGE_ELEM_CLASS).on("click keydown", function(e) {
        if ((e.type === 'keydown') &&  (e.keyCode !== 13)) return;
        prepareToChange($(this).parent().find('input.'+SCRIPT_PAGINATION_CHANGE_ELEM_CLASS).val());
    });
}
page.addGotoPage.nameForUser = 'Заменять в форуме поле для перехода на любую страницу';
...
page.addAditionalSearchToForum.nameForUser="Заменять(добавлять) в форуме стандартный поиск по теме";
...
var SCROLL_BUTTON_FILTER = "#scrollToTop";
/** Удаляет кнопку наверх */                                
page.removeUpButton = function() {
    
    if (page.MO) {
        var observer = new MutationObserver(function(mutations) {
            if ($(SCROLL_BUTTON_FILTER).length === 0) return;
            $(SCROLL_BUTTON_FILTER).remove();
            this.disconnect();
        });
        observer.observe($(BODY_FILTER)[0], {childList: true }); 
    }
    else { // если нет поддерржки MutationObserver сделаем 'по-старинке
...
function BankiruPage() {
    ...
    this.beforeHash =  window.location.href.replace(window.location.hash,'');
} 
